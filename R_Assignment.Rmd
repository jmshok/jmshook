...
for (i in 1:10) {
 ...
 write.csv(maize_loop, sprintf("maize_chromosome_%d_increasing_snps.txt", i), row.names = F)
}
```
...
for (i in 1:10) {
 ...
 write.csv(maize_loop, sprintf("maize_chromosome_%d_decreasing_snps.txt", i), row.names = F)
}
```
...
for (i in 1:10) {
 ...
 write.csv(teosinte_loop, sprintf("teosinte_chromosome_%d_increasing_snps.txt", i), row.names = F)}
```
...
for (i in 1:10) {
 ...
 write.csv(teosinte_loop, sprintf("teosinte_chromosome_%d_decreasing_snps.txt", i), row.names = F)}
```
...
write.csv(all_groups,"all_groups.csv", header=T)
write.csv(combined,"combined.csv", header=T)
#Had to manually merge column names using Excel- paste column names from all_groups starting with column 15 in combined
all_groups <- read.csv("combined.csv", header=T)
